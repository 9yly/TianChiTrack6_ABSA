...
OUTPUT_CSV_PATH   = "D:/python-ECI/competitions/tianchi/choose/Result_1031v11821.csv"         # 预测结果输出路径（提交格式）
RAW_JSONL_PATH    = "D:/python-ECI/competitions/tianchi/choose/raw_predictions_1031v11821.jsonl"  # 保存结构化结果（便于排查）
MODEL_NAME        = "ft:LoRA/Qwen/Qwen2.5-32B-Instruct:clzc5301v000e12rx905dx6mo:qcvunqozahym-ckpt_step_182"   
API_KEY           = "sk-slhfvynmzmwpwpgbmrshlivzkfkhb"  # 尽量用环境变量
USE_RESPONSE_FORMAT = False  # 如果将来 API 支持严格 JSON，可改 True
TEMPERATURE       = 0.0
MAX_TOKENS        = 4096
MAX_RETRIES       = 1          # 每条样本解析失败重试次数
SLEEP_BETWEEN     = 0.6        # 每次调用间隔秒数（防限流）
MAX_WORKERS       = 5        # 并发执行的线程数
...
def load_existing_predictions(jsonl_path: str) -> Dict[str, List[Quadruple]]:
    """
    加载已有的预测结果
                ...
                if quadruples_data:
                    ...
                    for q_data in quadruples_data:
                        try:
                            ...
                            continue
                    if quadruples:
                        predictions[review_id] = quadruples
...
def run():
    ...
    # 加载已有的预测结果
    print(f"正在加载已有预测结果: {RAW_JSONL_PATH}")
    existing_predictions = load_existing_predictions(RAW_JSONL_PATH)
    print(f"已加载 {len(existing_predictions)} 条成功预测的结果")
    ...
    # 筛选需要预测的样本（跳过已成功预测的）
    tests_to_process = []
    for rid, text in tests:
        if rid in existing_predictions:
            continue
        tests_to_process.append((rid, text))
    
    print(f"需要预测的样本数: {len(tests_to_process)}")
    print(f"跳过已预测样本数: {len(tests) - len(tests_to_process)}")

    if not tests_to_process:
        print("所有样本已预测完成，无需重新预测")
        # 使用已有预测生成最终结果
        all_predictions = [Prediction(review_id=rid, quadruples=quads) 
                          for rid, quads in existing_predictions.items()]
    ...
    # 以追加模式打开文件
    raw_f = open(RAW_JSONL_PATH, "a", encoding="utf-8")
    ...
    def process_wrapper(idx_rid_text):
        ...
        try:
            ...
            print(f"[{idx}/{len(tests_to_process)}] id={rid} 预测 {len(pred.quadruples)} 条")
            return pred
        except Exception as e:
            print(f"[ERROR] id={rid} 处理失败: {e}")
            return Prediction(review_id=rid, quadruples=[])
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            # 提交所有任务
            futures = {
                executor.submit(process_wrapper, (idx, rid, text)): (idx, rid)
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"[ERROR] 任务 id={rid} 执行异常: {e}")
    finally:
        raw_f.close()
    ...
    for rid, text in tests:
        if rid in existing_predictions:
            all_predictions.append(Prediction(review_id=rid, quadruples=existing_predictions[rid]))
        elif rid in new_predictions:
            all_predictions.append(new_predictions[rid])
        else:
            # 如果既不在已有预测中，也不在新预测中，说明预测失败
            all_predictions.append(Prediction(review_id=rid, quadruples=[]))
    
    write_result_csv(all_predictions, OUTPUT_CSV_PATH)
    print(f"完成：提交文件 -> {OUTPUT_CSV_PATH}")
    print(f"结构化备份 -> {RAW_JSONL_PATH}")
    print(f"本次新预测: {len(new_predictions)} 条")
    print(f"总计: {len(all_predictions)} 条")
